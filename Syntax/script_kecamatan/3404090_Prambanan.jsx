...
function Main() {
	...
	app.changeTextPreferences.changeTo = "Prambanan";
	document.changeText();
	...
	// change kokap to Prambanan in master page
	for (var i = 0; i < masterPages.length; i++) {
		...
		for (var j = 0; j < allTextFrames.length; j++) {
			...
			// if in textFrame include "Kokap" then change to "Prambanan"
			if (textFrame.contents.indexOf("Kokap") !== -1) {
				textFrame.contents = textFrame.contents.replace("Kokap", "Prambanan");
			}
		}
	}
