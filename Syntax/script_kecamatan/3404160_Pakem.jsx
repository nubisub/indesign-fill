...
function Main() {
	...
	app.changeTextPreferences.changeTo = "Pakem";
	document.changeText();
	...
	// change kokap to Pakem in master page
	for (var i = 0; i < masterPages.length; i++) {
		...
		for (var j = 0; j < allTextFrames.length; j++) {
			...
			// if in textFrame include "Kokap" then change to "Pakem"
			if (textFrame.contents.indexOf("Kokap") !== -1) {
				textFrame.contents = textFrame.contents.replace("Kokap", "Pakem");
			}
		}
	}
